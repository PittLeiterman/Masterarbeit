import numpy as np
from scipy.sparse import csr_matrix, vstack as sp_vstack, hstack as sp_hstack, block_diag as sp_block_diag
...
def stack_Q(segment_times):
    ...
    return sp_block_diag(Q_blocks, format="csr")
...
def precompute_mapping(segment_times, p0: float, pS: float, v_start: float, v_end: float):
    S = len(segment_times) - 1
    Q_blk = stack_Q(segment_times) * 2.0
